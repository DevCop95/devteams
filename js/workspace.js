// workspace.js - Browser Local Files Access (PDF, Excel, Word, TXT)
...
const _WORKSPACE_UPLOAD_TYPES=[{
  description:'Documentos y hojas',
  accept:{
    'application/pdf':['.pdf'],
...
async function _selectWorkspaceUploadFiles(){
  if(window.showOpenFilePicker){
    try{
      const handles=await window.showOpenFilePicker({multiple:true,types:_WORKSPACE_UPLOAD_TYPES});
      return Promise.all(handles.map(async handle=>({name:handle.name,file:await handle.getFile()})));
    }catch(e){
      if(/abort/i.test(`${e?.name||''} ${e?.message||''}`))return [];
      throw e;
    }
  }
  return new Promise(resolve=>{
    ...
    input.type='file';
    input.multiple=true;
    input.accept='.txt,.md,.pdf,.docx,.doc,.xlsx,.xls,.json,.csv,.js,.ts,.tsx';
    ...
    const done=(files)=>{
      ...
      finished=true;
      input.remove();
      resolve(files.map(file=>({name:file.name,file})));
    };
    input.addEventListener('change',()=>done(Array.from(input.files||[])));
    input.addEventListener('blur',()=>setTimeout(()=>{
      if(!finished && !input.files?.length)done([]);
    },250),{once:true});
    input.click();
  });
}

async function _uniqueWorkspaceFileName(dirHandle,name){
  ...
  while(true){
    try{
      ...
      const reason=`${e?.name||''} ${e?.code||''} ${e?.message||''}`;
      if(/notfound/i.test(reason))return candidate;
      throw e;
    }
  }
}

async function _uploadWorkspaceFiles(){
  if(!_workspaceDirHandle){
    await _pickWorkspaceFolder();
  }
  const selections=await _selectWorkspaceUploadFiles();
  if(!selections.length)return {message:'No se seleccionaron archivos.',files:[]};
  const uploads=await _workspaceDirHandle.getDirectoryHandle('uploads',{create:true});
  const created=[];
  for(const item of selections){
    const name=item.name||'documento';
    const targetName=await _uniqueWorkspaceFileName(uploads,name);
    const dest=await uploads.getFileHandle(targetName,{create:true});
    const writable=await dest.createWritable();
    await writable.write(item.file);
    await writable.close();
    const rel=`uploads/${targetName}`;
    created.push(rel);
    _rememberWorkspaceFile(rel);
  }
  await _indexWorkspace(true);
  const humanList=created.map(p=>_workspaceDisplayName(p)).join(', ');
  return {
    message:`Subidos ${created.length} archivos a uploads/: ${humanList}`,
    files:created
  };
}
...
function _workspaceToolHelp(){
  return [
    ...
    '/subir -> subir nuevos documentos al folder uploads del workspace',
...
async function _handleBrowserToolCommand(rawText, agentKey, userLabel = '') {
  ...
  if (text.toLowerCase().includes('mejorar') || text.toLowerCase().includes('cambiar')) {
    ...
    sharedHint.style.cssText = 'font-size:15px;color:var(--t3);padding:4px 10px;background:rgba(200,160,64,0.1);border-left:3px solid #c8a040;margin:6px 0';
  ...
  if (/^\/subir\b/i.test(text)) {
    try {
      const result = await _uploadWorkspaceFiles();
      if (result.files.length) {
        updateSharedDecision(agentKey, `Subió ${result.files.length} documentos: ${result.files.map(f => _workspaceDisplayName(f)).join(', ')}`);
      }
      return reply(result.message);
    } catch (e) {
      return reply(`Error al subir archivos: ${e.message}`);
    }
  }
...
async function workspaceQuickAction(kind){
  ...
  if(kind==='upload'){
    try{
      ...
      if(result.files.length){
        updateSharedDecision(agentKey,`Subió ${result.files.length} documentos: ${result.files.map(f=>_workspaceDisplayName(f)).join(', ')}`);
        _workspaceToolReply(agentKey,'Tools > Subir',result.message);
        
        if(result.files.length === 1){
          const uploadedFile = result.files[0];
          _setWorkspaceQuickQuery(uploadedFile);
          return _handleBrowserToolCommand(`/analizar ${uploadedFile}`, agentKey, `Tools > Analizar ${_workspaceDisplayName(uploadedFile)}`);
        } else {
          // Si son múltiples, abrir el selector para que decida cuál analizar
          return openWorkspaceFilePicker('analyze');
        }
      } else {
        return _workspaceToolReply(agentKey,'Tools > Subir',result.message);
      }
    }catch(e){
      return _workspaceToolReply(agentKey,'Tools > Subir',`Error al subir archivos: ${e?.message||'error'}`);
    }
  }
...
window._analyzeWorkspaceFile = _analyzeWorkspaceFile;
window._appendWorkspaceInteractiveList = _appendWorkspaceInteractiveList;
window._downloadTextFile = _downloadTextFile;
window._ensureOfficeReaders = _ensureOfficeReaders;
window._ensurePdfJs = _ensurePdfJs;
window._findWorkspaceFile = _findWorkspaceFile;
window._formatWorkspaceList = _formatWorkspaceList;
window._handleBrowserToolCommand = _handleBrowserToolCommand;
window._indexWorkspace = _indexWorkspace;
window._isExcelFile = _isExcelFile;
window._isPdfFile = _isPdfFile;
window._isTextFile = _isTextFile;
window._isWordFile = _isWordFile;
window._normalizeWorkspacePath = _normalizeWorkspacePath;
window._pickWorkspaceFolder = _pickWorkspaceFolder;
window._readWorkspaceExcel = _readWorkspaceExcel;
window._readWorkspacePdf = _readWorkspacePdf;
window._readWorkspaceText = _readWorkspaceText;
window._readWorkspaceWord = _readWorkspaceWord;
window._refreshWorkspaceQuickUI = _refreshWorkspaceQuickUI;
window._rememberWorkspaceFile = _rememberWorkspaceFile;
window._searchWorkspaceText = _searchWorkspaceText;
window._selectWorkspaceUploadFiles = _selectWorkspaceUploadFiles;
window._setWorkspaceQuickQuery = _setWorkspaceQuickQuery;
window._uniqueWorkspaceFileName = _uniqueWorkspaceFileName;
window._uploadWorkspaceFiles = _uploadWorkspaceFiles;
window._walkWorkspace = _walkWorkspace;
window._workspaceDisplayName = _workspaceDisplayName;
window._workspaceInteractiveSummary = _workspaceInteractiveSummary;
window._workspaceKindLabel = _workspaceKindLabel;
window._workspacePickerTag = _workspacePickerTag;
window._workspaceQuickQueryValue = _workspaceQuickQueryValue;
window._workspaceReadMessage = _workspaceReadMessage;
window._workspaceToolAgentKey = _workspaceToolAgentKey;
window._workspaceToolHelp = _workspaceToolHelp;
window._workspaceToolReply = _workspaceToolReply;
window._workspaceViewerMeta = _workspaceViewerMeta;
window.chooseWorkspaceFilePickerItem = chooseWorkspaceFilePickerItem;
window.closeWorkspaceFilePicker = closeWorkspaceFilePicker;
window.closeWorkspaceViewer = closeWorkspaceViewer;
window.filterWorkspaceFilePicker = filterWorkspaceFilePicker;
window.openWorkspaceFilePicker = openWorkspaceFilePicker;
window.openWorkspacePath = openWorkspacePath;
window.openWorkspaceViewer = openWorkspaceViewer;
window.workspaceQuickAction = workspaceQuickAction;
window.workspaceViewerAction = workspaceViewerAction;
window._WORKSPACE_UPLOAD_TYPES = _WORKSPACE_UPLOAD_TYPES;
